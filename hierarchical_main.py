...
# from modelling.design2_model import DataWithIndices as Data
...
def global_modeling(df: pd.DataFrame, label: str):
    print(f"\n Global Model for {label} (Intent)")
    Config.CLASS_COL = label
    df["y"] = df[Config.CLASS_COL]
    X_global = get_tfidf_embd(df, min_df=1, max_df=1.0)
    data_global = Data(X_global, df)
    if data_global.X_train is None:
        print(f" Skipping {label} — not enough data.")
        return

    model = RandomForest(f"RF_{label}_global", data_global.get_embeddings(), data_global.get_type())
    model.train(data_global)
    model.predict(data_global.X_test)

    df_test = data_global.df_filtered.loc[data_global.get_test_indices()].copy()
    print(f" {label} test samples: {len(df_test)}")
    df_test[f"{label}_true"] = data_global.y_test
    df_test[f"{label}_pred"] = model.predictions

    print(f"\n Classification Report for {label} (Intent)")
    print(classification_report(df_test[f"{label}_true"], df_test[f"{label}_pred"], zero_division=0))

def train_filtered_hierarchical_models(df: pd.DataFrame):
    print("\n Starting Filtered Hierarchical Modelling (Model Tree)")
    all_rows = []

    global_modeling(df, 'y2')
    global_modeling(df, 'y3')
    global_modeling(df, 'y4')
    ...
    for y2_class in df_y2_test["y2_pred"].unique():
        ...
        if data_y3.X_train is None or len(data_y3.X_test) == 0 or pd.isnull(data_y3.y_train).any():
            continue
        ...
        for y3_class in df_y3_test["y3_pred"].unique():
            ...
            if data_y4.X_train is None or len(data_y4.X_test) == 0 or pd.isnull(data_y4.y_train).any():
                continue
